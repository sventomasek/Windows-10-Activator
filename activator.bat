@echo off
title Windows 10 activator&cls&echo ------------------------------------------&echo Supported Products:&echo - Windows 10 Home&echo - Windows 10 Professional&echo - Windows 10 Enterprise, Enterprise LTSB&echo - Windows 10 Education&echo ------------------------------------------ &echo Windows 10 activation...
...
cscript //nologo c:\windows\system32\slmgr.vbs /ato | find /i "successfully" && (echo ------------------------------------------ & echo. & choice /n /c YN /m "Restart now [Y,N]?" & if errorlevel 2 exit) || (echo The connection to the server failed! Trying to connect to another one... & echo Please wait... & echo. & echo. & set /a i+=1 & goto server)
shutdown.exe /r /t 00
